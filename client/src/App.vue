  ...
  <div class="p-6 max-w-xl mx-auto text-center text-stone-700 text-3xl">
    ...
    <div v-if="!nameSubmitted" class="space-y-8">
      <div class="space-y-4">
        <h1 class="text-5xl">
          <span
            :class="`text-${previewColor}-700 block text-8xl font-display uppercase tracking-widest transition-colors duration-150 ease-out`">Who</span>
          wants to play?
        </h1>

        <p class="text-2xl text-stone-500 leading-6 text-balance">A game of questionable accusations for 3-10 suspects.
        </p>
      </div>
      ...
      <form v-if="!lobbyFull" @submit.prevent="submitName" class="space-y-8">
        ...
        <ul role="radiogroup" aria-label="Choose a color" class="flex flex-wrap justify-center gap-3">
          <li v-for="color in colors" :key="color">
            <button type="button" role="radio" @click="pickColor(color)" :disabled="!colorAvailable(color)"
              ...
              <svg v-if="previewColor === color" class="w-5 h-5 fill-white" viewBox="0 0 512 512"
        ...
        <button type="submit" :disabled="joinDisabled" :class="[
          ...
          joinDisabled
            ? 'bg-stone-300 text-stone-400 border-transparent cursor-not-allowed translate-y-0.75'
            : `bg-${previewColor}-600 text-white border-${previewColor}-800 cursor-pointer hover:bg-${previewColor}-700 hover:border-transparent hover:translate-y-0.75`
        ]">
          Join
        </button>
      </form>
    ...
    <div v-else class="space-y-6">
      <div v-if="currentQuestion" class="space-y-12">
        ...
        <div class="space-y-6">
          ...
          <!-- Countdown Timer -->
          <p v-if="countdown"
            :class="['text-2xl flex items-center justify-center gap-2', countdown <= 5 ? 'text-red-700' : 'text-stone-500']">
            ...
            <ul v-if="votingAllowed" class="space-y-2">
              <li v-for="player in playerList" :key="player.socketId">
                <button @click="vote(player.socketId)" :class="[
                  'flex items-center gap-3 justify-center px-8 py-4 rounded-full w-full border-b-4 text-white cursor-pointer',
                  'transition-all duration-150 ease-out',
                  player.socketId === myCurrentVoteId
                    ? `bg-${player.color}-700 border-transparent`
                    : `bg-${player.color}-600 border-${player.color}-800 hover:bg-${player.color}-700 hover:border-transparent hover:translate-y-0.75`
                ]">
      ...
      <div v-if="!gameActive" class="space-y-8">
        ...
        <button v-if="canStart && !waiting" @click="startGame"
          :class="`bg-${playerColor}-600 border-b-4 border-${playerColor}-800 text-white px-8 py-4 rounded-full w-full cursor-pointer transition-all duration-150 ease-out hover:bg-${playerColor}-700 hover:border-transparent hover:translate-y-0.75`">
          Start Game
        </button>
      </div>
    </div>
